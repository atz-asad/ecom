...
@section('content')
  <div class="page-header">
    ...
    <ul class="breadcrumbs">
      ...
      <li class="nav-item">
        <a
          href="{{ route('user.blog.category.index') . '?language=' . request()->input('language') }}">{{ __('Categories') }}</a>
      </li>
      <li class="separator">
        <i class="flaticon-right-arrow"></i>
      </li>
      ...
      <div class="card">
        <div class="card-header">
          ...
          <a class="btn btn-info btn-sm float-right d-inline-block"
            href="{{ route('user.blog.category.index') . '?language=' . request()->input('language') }}">
        ...
        <div class="card-body pt-5 pb-5">
          <div class="row">
            <div class="col-lg-6 m-auto">
              <form id="ajaxForm" action="{{ route('user.blog.category.update') }}" method="POST">
                @csrf
                @foreach ($languages as $language)
                  @php
                    $category = App\Models\User\BlogCategory::where([
